...
| Agent | `<three words>` | `au jsfoptions schema` |
| Workspace | `<label> - <three words>` | `work-1 - au jsfoptions schema` |
...
current step), abbreviated to three words. Measured latency for that call is
12-28s (median ~24s), so it can't run synchronously in the event hook: the
hook renames the tab immediately and spawns the agent/workspace naming as a
detached background process, which applies the rename to herdr about 24s
later once Haiku responds. `claude -p` is invoked with `--system-prompt`,
`--setting-sources ""`, and a neutral cwd so it doesn't inherit ambient
project context — without that it was leaking the repo's own name into
generated labels.
...
- Naming calls that don't return within 45s time out; the last good name is
  kept with a `…` marker (`work-1 - au jsfoptions schema …`) and retried on
  the next event.
